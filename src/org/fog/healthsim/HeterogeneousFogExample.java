...
public class HeterogeneousFogExample {
    ...
    private static void createHeterogeneousFogDevices(int userId) {
        ...
        for(int i = 0; i < numOfAreas; i++) {
            FogDevice edgeDataCenter = createFogDevice("edge-dc-"+i, 2800, 4000, 10000, 10000, 1, 0.0, 107.339, 83.4333);
            edgeDataCenter.setParentId(cloud.getId());
            edgeDataCenter.setUplinkLatency(100);
            fogDevices.add(edgeDataCenter);
            
            FogDevice fogGateway = createFogDevice("fog-gateway-"+i, 2800, 4000, 10000, 270, 2, 0.0, 107.339, 83.4333);
            ...
            for(int j = 0; j < numOfDevicesPerArea; j++) {
                FogDevice fogNode = createFogDevice("fog-node-"+i+"-"+j, 1000, 1000, 10000, 270, 3, 0.0, 87.53, 82.44);
                fogNode.setParentId(fogGateway.getId());
                fogNode.setUplinkLatency(2);
                fogDevices.add(fogNode);
                
                FogDevice iotDevice = createFogDevice("iot-device-"+i+"-"+j, 500, 1000, 10000, 270, 4, 0.0, 87.53, 82.44);
    ...
    private static void createSensorsAndActuators(int userId, String appId1, String appId2, String appId3) {
        ...
        for(FogDevice device : fogDevices) {
            if(device.getName().startsWith("iot-device")) {
                ...
                Actuator trafficLightActuator = new Actuator("traffic-light-"+deviceIndex, userId, appId3, "TRAFFIC_CONTROL");
    ...
    private static Application createIoTMonitoringApplication(String appId, int userId) {
        ...
        application.addAppEdge("temp_processor", "data_analyzer", 2000, 2000, "PROCESSED_TEMP", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("data_analyzer", "alert_manager", 100, 1000, "ANALYSIS_RESULT", Tuple.UP, AppEdge.MODULE);
        ...
        application.addTupleMapping("data_analyzer", "PROCESSED_TEMP", "ANALYSIS_RESULT", new FractionalSelectivity(0.05));
        application.addTupleMapping("alert_manager", "ANALYSIS_RESULT", "HVAC_COMMANDS", new FractionalSelectivity(0.1));
    ...
    private static Application createVideoStreamingApplication(String appId, int userId) {
        ...
        application.addAppEdge("motion_detector", "object_detector", 2000, 1000, "MOTION_VIDEO_STREAM", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("object_detector", "video_encoder", 1000, 1000, "DETECTED_OBJECTS", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("video_encoder", "DISPLAY", 1000, 1000, "PROCESSED_VIDEO", Tuple.DOWN, AppEdge.ACTUATOR);
        
        application.addTupleMapping("motion_detector", "RAW_VIDEO", "MOTION_VIDEO_STREAM", new FractionalSelectivity(0.9));
        application.addTupleMapping("object_detector", "MOTION_VIDEO_STREAM", "DETECTED_OBJECTS", new FractionalSelectivity(1.0));
        application.addTupleMapping("video_encoder", "DETECTED_OBJECTS", "PROCESSED_VIDEO", new FractionalSelectivity(1.0));
    ...
    private static Application createSmartCityApplication(String appId, int userId) {
        ...
        application.addAppEdge("TRAFFIC_DATA", "traffic_analyzer", 2000, 2000, "TRAFFIC_INFO", Tuple.UP, AppEdge.SENSOR);
        application.addAppEdge("traffic_analyzer", "city_coordinator", 1000, 1000, "TRAFFIC_ANALYSIS", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("city_coordinator", "decision_engine", 500, 1000, "CITY_STATUS", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("decision_engine", "TRAFFIC_CONTROL", 100, 28, "CONTROL_COMMANDS", Tuple.DOWN, AppEdge.ACTUATOR);
        
        application.addTupleMapping("traffic_analyzer", "TRAFFIC_INFO", "TRAFFIC_ANALYSIS", new FractionalSelectivity(1.0));
        application.addTupleMapping("city_coordinator", "TRAFFIC_ANALYSIS", "CITY_STATUS", new FractionalSelectivity(0.1));
        application.addTupleMapping("decision_engine", "CITY_STATUS", "CONTROL_COMMANDS", new FractionalSelectivity(0.05));
    ...
    private static void configureModulePlacement(Controller controller, Application iotApp, Application videoApp, Application smartCityApp) {
